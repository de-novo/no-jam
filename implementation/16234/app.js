...
class World {
    ...
    getUnion(country, union = [], visit = []) {
        visit[this.getCountryIndex(country)] = 1;

        while (1) {
            const nearCountry = this.getNearCountry(country).filter(
                (c) =>
                    this.compare(country, c) && !visit[this.getCountryIndex(c)]
            );
            if (nearCountry.length === 0) break;
            nearCountry.forEach((c) => {
                if (
                    this.compare(country, c) &&
                    !visit[this.getCountryIndex(c)]
                ) {
                    union.push(c);
                    visit[this.getCountryIndex(c)] = 1;
                    this.getUnion(c, union, visit);
                }
            });
        }
        const checkLocation = union.map((c) => this.getCountryIndex(c));
        return [union, checkLocation];
    }
